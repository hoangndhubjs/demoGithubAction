...
class Menu
{
    ...
    {
        # Check permission for show item;
        /*if(isset($item['permission'])) return;*/
        self::checkRecursion($rec);
        if (!$item) { return 'menu misconfiguration'; }
        if (isset($item['role']) && $item['role'] === 'leader' && Auth::user()->staffs->count() < 1) {
            return;
        }
        if (isset($item['separator'])) {
            ...
            if (isset($item['submenu']) && self::isRouteActive($item)) {
                $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
            } elseif (self::isRouteActive($item)) {
    ...
    //Check route active;
    public static function isRouteActive($item) {
        $current_route_name = Route::currentRouteName();
        $active_routes = $item['active_routes'] ?? array();
        return in_array($current_route_name, $active_routes);
    }

    public static function isSubRouteActive($item) {
        $current_route_name = Route::currentRouteName();
        return Str::startsWith($current_route_name, $item);
    }
