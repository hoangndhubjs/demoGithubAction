...
@section('content')
    ...
    <div class="modal fade px-0" id="popupImmigration" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="popupImmigration" aria-hidden="true" data-form-url="{{ route('employee_managements.ajax.create_form_immigration', request()->route('id'))}}">
...
@section('scripts')
    <script type="text/javascript">
        ...
        window._tables = {
            'immigration': {
                'url': '{{ route('employee_managements.immigrationList', request()->route('id')) }}',
                'columns': [
                    {
                        field: 'stt',
                        title: '{{ __('STT') }}',
        ...
        function deleteRequest(id) {
            Swal.fire({
                ...
                preConfirm: function (data) {
                    ...
                    $.ajax({
                        method: 'DELETE',
                        url: '{{ route("employee_managements.deleteImmigration") }}',
                        data: { id: id }
                    }).done(function(response) {
                        if (response.success) {
                            Swal.fire(
