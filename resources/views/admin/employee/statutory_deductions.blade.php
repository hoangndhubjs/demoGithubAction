...
{{--                            <span class="d-block text-muted pt-2 font-size-sm">{{__('my_list_compensations')}}</span></h3>--}}
                    ...
                    <div class="card-toolbar">
                        <!--begin::Button-->
                        <div>
                            <a href="#" class="btn btn-primary font-weight-bolder" data-toggle="modal" data-target="#deductions_form">
            ...
            <!--end::Modal-->
            <!-- begin: Modal -->
            <div class="modal fade" id="deductions_form" tabindex="-1" role="dialog" aria-labelledby="deductions_form" aria-hidden="true" data-form-url="{{ route('employee_managements.createFormDeductions')}}">
...
@section('scripts')
    <script>
        window._tables = {
            'deductions_list': {
                'url': '{{ route('employee_managements.ajax.list_deductions') }}?user_id='+{{ request()->route('id') }},
                'columns': [
                    {
                        field: 'STT',
                        title: '{{ __('STT') }}',
                    ...
                    },
                    {
                        ...
                        title: '{{ __('deduction_type') }}',
                        sortable: false,
                        textAlign: 'center',
                        template: function (row) {
                            return row.deduction_title
                        }
                    },
                    {
                        field: 'basic_salary_region',
                        title: '{{ __('basic_salary_region') }}',
                        sortable: false,
                        textAlign: 'center',
                        template: function (row) {
                            return window._userCurrency(parseInt(row.basic_salary_region))
                        }
                    },
                    {
                        ...
                        textAlign: 'center',
                        template: function (row) {
                            return window._userCurrency(parseInt(row.deduction_amount))
                        }
                    },
                    {
                        field: 'deductions',
                        ...
                        template: function (row) {
                            ...
                            return allowance_taxable[row.statutory_options].text
                        }
                    },
                    {
        ...
        $('#deductions_form').on('shown.bs.modal', function (e) {
            window._loading("#deductions_form .modal-body");
            let button = $(e.relatedTarget);
            let id = button.data('id');
            let url = $('#deductions_form').data('form-url');
            $.ajax({
                url: url,
                data: {
                    id: id,
                ...
                window._loading("#deductions_form .modal-body", false);
                $("#deductions_form .modal-body").html(response);
            }).fail(function (jqXHR, status){
                window._loading("#deductions_form .modal-body", false);
                toastr.error(__("order_form_fetch_error"));
                window._display_alert("#deductions_form .modal-body", __("order_form_fetch_error"));
            });
        });
        $('#deductions_form').on('hidden.bs.modal', function (e) {
            $("#deductions_form .modal-body").html("");
        });
