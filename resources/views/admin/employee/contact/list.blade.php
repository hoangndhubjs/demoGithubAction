...
@section('content')
    ...
    <div class="modal fade px-0" id="popupContact" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="popupContact" aria-hidden="true" data-form-url="{{ route('employee_managements.ajax.create_form_contact', request()->route('id'))}}">
...
@section('scripts')
    <script type="text/javascript">
        ...
        window._tables = {
            'contact': {
                'url': '{{ route('employee_managements.contactList', request()->route('id')) }}',
                'columns': [
                    {
                        field: 'stt',
                        title: '{{ __('STT') }}',
        ...
        function deleteRequest(id) {
            Swal.fire({
                ...
                preConfirm: function (data) {
                    ...
                    $.ajax({
                        method: 'DELETE',
                        url: '{{ route("employee_managements.deleteContact") }}',
                        data: { id: id }
                    }).done(function(response) {
                        if (response.success) {
                            Swal.fire(
