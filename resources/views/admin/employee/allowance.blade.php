...
{{--                            <span class="d-block text-muted pt-2 font-size-sm">{{__('my_list_compensations')}}</span></h3>--}}
                    ...
                    <div class="card-toolbar">
                        <!--begin::Button-->
                        <div>
                            <a href="#" class="btn btn-primary font-weight-bolder" data-toggle="modal" data-target="#allowance_form">
                ...
                <div class="card-body card-body-compensations">
                    ...
                    <!--end::Search Form-->
                    <!--begin: Datatable-->
                    <div class="datatable datatable-bordered datatable-head-custom" id="allowance_list"></div>
                    <!--end: Datatable-->
                </div>
            </div>
        <!--end::Modal-->
            <!-- begin: Modal -->
            <div class="modal fade" id="allowance_form" tabindex="-1" role="dialog" aria-labelledby="allowance_form" aria-hidden="true" data-form-url="{{ route('employee_managements.ajax.create_form')}}">
                <div class="modal-dialog modal-dialog-centered dialog_compensations" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ __("money_plus") }}</h5>
...
@section('scripts')
    <script>
        window._tables = {
            'allowance_list': {
                'url': '{{ route('employee_managements.ajax.lists') }}?user_id='+{{ request()->route('id') }},
                'columns': [
                    {
                        field: 'STT',
                        title: '{{ __('STT') }}',
                    ...
                    },
                    {
                        ...
                        template: function (row) {
                            return row.allowance_title
                        }
                    },
                    {
                        field: 'number_allowance',
                        title: '{{ __('xin_amount') }}',
                        sortable: false,
                        textAlign: 'center',
                        template: function (row) {
                            return window._userCurrency(parseInt(row.allowance_amount))
                        }
                    },
                    {
                        field: 'deductions',
                        ...
                        template: function (row) {
                            ...
                            return allowance_taxable[row.is_allowance_taxable].text
                        }
                    },
                    {
                        field: 'xin_amount_option',
                        title: '{{ __('xin_amount_option') }}',
                        sortable: false,
                        textAlign: 'center',
                        // template: function (row) {
                        ...
                        template: function (row) {
                            ...
                            return status[row.amount_option].text
        ...
        $('#allowance_form').on('shown.bs.modal', function (e) {
            window._loading("#allowance_form .modal-body");
            let button = $(e.relatedTarget);
            let id = button.data('id');
            let url = $('#allowance_form').data('form-url');
            $.ajax({
                url: url,
                data: {
                    id: id,
                ...
                window._loading("#allowance_form .modal-body", false);
                $("#allowance_form .modal-body").html(response);
            }).fail(function (jqXHR, status){
                    window._loading("#allowance_form .modal-body", false);
                    toastr.error(__("order_form_fetch_error"));
                    window._display_alert("#allowance_form .modal-body", __("order_form_fetch_error"));
                });
        });
        $('#allowance_form').on('hidden.bs.modal', function (e) {
            $("#allowance_form .modal-body").html("");
        });
            ...
            if (id) {
                Swal.fire({
                    ...
                    preConfirm: function (data) {
                        ...
                        $.ajax({
                            method: 'DELETE',
                            url: '{{ route("employee_managements.ajax.delete_allowance") }}',
                            data: {id: id}
                        }).done(function (response) {
                            if (response == true) {
                                Swal.fire(
