...
@section('content')
    ...
    <div class="modal fade px-0" id="popupExperience" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="popupExperience" aria-hidden="true" data-form-url="{{ route('employee_managements.ajax.create_form_experience', request()->route('id'))}}">
...
@section('scripts')
    <script type="text/javascript">
        ...
        window._tables = {
            'experience': {
                'url': '{{ route('employee_managements.ExperienceList', request()->route('id')) }}',
                'columns': [
                    {
                        field: 'stt',
                        title: '{{ __('STT') }}',
        ...
        function deleteRequest(id) {
            Swal.fire({
                ...
                preConfirm: function (data) {
                    ...
                    $.ajax({
                        method: 'DELETE',
                        url: '{{ route("employee_managements.deleteExperience") }}',
                        data: { id: id }
                    }).done(function(response) {
                        if (response.success) {
                            Swal.fire(
