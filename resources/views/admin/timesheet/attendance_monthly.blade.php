...
<div class="row">
    <div class="col-xl-3 mb-5">
        <div class="card card-custom card-stretch">
            ...
            <div class="card-body" style="flex: none;">
                ...
                <div class="attendance-leave font-weight-bold p-3 mb-3 rounded">{{ __("attendance_leave") }}: V1 - V2</div>
    ...
    <div class="col-xl-9">
        <div class="card card-custom card-stretch">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">{{ __("attendance_monthly_list").' '. $employee->first_name.' '. $employee->last_name }}
                        <span class="d-block text-muted pt-2 font-size-sm"></span>
                    </h3>
                </div>
            </div>
...
@section('scripts')
    <script type="text/javascript">
        window._start_date = '{{$month.'-01'}}';
        window._end_date = '{{$month.'-'.date('t', strtotime($month.'-01'))}}';
        window._attendance_monthly_url = '{{ route("admin.timesheet.ajax.attendance_monthly.list", ['id'=>$id]) }}';
        window._attendance_monthly_summary_url = '{{ route('admin.timesheet.ajax.attendance_monthly.summary', ['id'=>$id]) }}';
    </script>
    <script src="{{ mix('js/fullcalendar.bundle.js') }}"></script>
	<script src="{{ mix('js/attendances/admin_attendance_month.js') }}"></script>
@endsection
