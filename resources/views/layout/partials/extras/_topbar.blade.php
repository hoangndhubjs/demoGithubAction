...
<div class="topbar">
    ...
    @if (config('layout.extras.notifications.display'))
        ...
        @else
            <div class="dropdown">
                {{-- Toggle --}}
                <div class="topbar-item" data-toggle="dropdown" data-offset="10px,0px">
                    <div class="btn btn-icon btn-clean btn-dropdown btn-lg mr-1 @if(\App\Classes\Notification::totalNotification() > 0) {{'pulse'}} @endif pulse-primary">
                        {{ Metronic::getSVG("media/svg/icons/General/Notifications1.svg", "svg-icon-xl svg-icon-primary") }}
                        <span class="pulse-ring" style="/*color: red; font-weight: bold*/">{{--{{ \App\Classes\Notification::totalNotification() }}--}}</span>
                    </div>
                </div>
    ...
    @if (config('layout.extras.user.display'))
        ...
        @if (config('layout.extras.user.layout') == 'offcanvas')
            <div class="topbar-item">
                @include('components.top_header_profile', ['useOffcanvas' => config('layout.extras.user.layout') == 'offcanvas'])
            </div>
        @else
            <div class="dropdown">
                {{-- Toggle --}}
