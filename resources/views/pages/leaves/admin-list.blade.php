...
@section('scripts')
    <script>
        ...
        window._tables = {
            'leaves_list': {
                'url': '{{ route('leaves.admin.ajax.list') }}',
                'columns': [
                    {
                        field: 'RecordID',
                        title: '#',
                    ...
                    },
                    {
                        ...
                        template: function (row) {
                            ...
                            if (row.status == 2 && row.confirm != 1) {
                                html += "<a href='#' onclick='confirmRequest(" + row.leave_id + ", 1)' class='btn btn-sm btn-clean btn-icon' title='{{ __('confirm') }}'>" + window.iconRec + "</a>";
                            }/* else if(row.status == 2 && row.confirm == 1) {
                                html += "<a href='#' onclick='confirmRequest("+row.leave_id+", 0)' class='btn btn-sm btn-clean btn-icon' title='{{ __('not_confirm') }}'>"+'<x-icon type="flaticon2" icon="cancel-music" class="text-danger"/>'+"</a>";
                            }*/
                            if(row.status == 1){
                            ...
                            /*if(row.status == 1){
                                html += "<a href='#' data-toggle='modal' data-target='#complaint_request' data-id='"+row.leave_id+"' class='btn btn-sm btn-clean btn-icon' title='{{ __('edit') }}'>"+'<x-icon type="flaticon2" icon="check-mark" class="text-primary"/>'+"</a>";
                            }*/
                            if (row.employee_id == '{{auth()->user()->user_id}}' && row.status == 1 && row.confirm == 0) {
        ...
        let confirmRequest = function (id, confirm) {
            Swal.fire({
                ...
                preConfirm: function (data) {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '{{ route("leaves.admin.ajax.confirm") }}',
